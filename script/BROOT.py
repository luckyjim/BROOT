...
def manage_menu(s_but):
    ...
    if s_but == "CLOSE":
        ...
        g_d_tables = {}
    if s_but == "OPEN":
        r_file = g_app.openBox(
            "BROOT open ROOT file",
            dirName=g_path_file,
            fileTypes=[("ROOT", "*.root"), ("ROOT", "*.r"), ("all", "*.*")],
        )
        if len(r_file) != 0:
...
def manage_action(s_but, i_line):
    ...
    def destroy():
        g_app.destroyTextArea("PRT")
        g_app.destroySubWindow("SUB_PRT")
        g_app.topLevel.clipboard_clear()

    def copy_print():
        g_app.topLevel.clipboard_append(str_a)
    ...
    if s_but.find("Print") >= 0:
        ...
        try:
            ...
            pass
        g_app.startSubWindow("SUB_PRT", f"Print {ttree}.{branch}", modal=True)
        g_app.addTextArea("PRT", text=str_a)
        g_app.addButtons(["Ok", "Copy"], [destroy, copy_print])
        g_app.stopSubWindow()
        g_app.showSubWindow("SUB_PRT")
